...
if [ $? -ne 0 ]; then
    print Making one_resume Virtual env failed
    exit -1
fi

workon one_resume
if [ $? -ne 0 ]; then
    print Could not switch to one_resume
    exit -1
fi
print Working in virtualenv one_resume


# Set up the pip packages
#pip install pytest mock pytest-cov python-coveralls coverage sphinx tox
pip install sphinx
echo "cd ~/dev/one_resume" >> ~/dev/envs/one_resume/bin/postactivate

# Start python develop
python setup.py develop

# Initialize the git repo
github_remote='git@github.com:virantha/one_resume.git'
git init
git remote add origin $github_remote
git add .
git commit -am "Setting up new project one_resume"

# Prompt if we want to push to remote git
read -q "REPLY?Create remote repository at $github_remote [y/N]?"
if [[  $REPLY == y ]]; then
    curl --data '{"name":"one_resume", "description":""}' --user "virantha" https://api.github.com/user/repos
fi
...
if [[  $REPLY == y ]]; then
    ...
    cd $DOCS_DIR
    git clone https://github.com/virantha/one_resume.git
    cd one_resume
