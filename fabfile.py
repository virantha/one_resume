...
def first_setup():
    ...
    local("mkvirtualenv one_resume")

    # pip install packages
    with prefix('workon one_resume'):
        local("pip install pytest")

def build_windows_dist():
    if os.name == 'nt':
        # Call the pyinstaller
        local("python ../pyinstaller/pyinstaller.py one_resume_windows.spec --onefile")
...
def run_tests():
    ...
    with lcd(test_dir):
        ...
        t = local("py.test --cov-config .coveragerc --cov=one_resume --cov-report=term --cov-report=html", capture=False)
...
def push_docs():
    ...
    """
    githubpages = "/Users/virantha/dev/githubdocs/one_resume"
